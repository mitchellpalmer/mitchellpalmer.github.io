...
def get_electorate_details(id):
    p = requests.get(f'https://www.electionresults.govt.nz/electionresults_2020_preliminary/electorate-details-{str(id).zfill(2)}.html')
    soup = BeautifulSoup(p.content, 'html.parser')

    print(id)
    electorate = soup.select_one('.page-title h2').string.replace(' - Preliminary Count', '')
